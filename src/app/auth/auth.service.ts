...
export class AuthService {
  ...
  autoLogin() {
    const userData = JSON.parse(localStorage.getItem('userData'));  // retrive and convert JSON back to javascript object
    if (!userData) {
      return;
    }
  ...
  logout() {
    // this.user.next(null); // Replaced by NgRx
    this.store.dispatch(new AuthActions.Logout()); // Replaces code above
  ...
  autoLogout(expirationDuration: number) {
   this.tokenExpirationTimer = setTimeout(() => {
      this.logout();
    }, expirationDuration) // log out automatically after a set amount of time
  }
